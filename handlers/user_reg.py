...
async def user_register(message: types.Message):
    await message.answer('Вітаємо! Зареєструйтесь, будь ласка. Це не забере багато часу.\n'
                         'Просто введіть своє імя, номер телефону та адресу. Це потрібно, щоб бот запам`ятав вас.\n'
                         '/esc - якщо ви потрапили сюда помилково')
    await message.answer("Будь ласка, введіть своє ім'я")
...
async def get_name(message: types.Message, state: FSMContext):
    ...
    await FSMuser.next()  # либо же UserState.adress.set()


async def get_phone_number(message: types.Message, state: FSMContext):
    await state.update_data(phone_number=message.text)
    await message.answer("Чудово! Залишилось додати адресу і все готово.")
    await FSMuser.next()
...
async def get_address(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"Ми майже зареєстрували вас, {data['name']}!\n"
                         f"Ваш номер телефону: {data['phone_number']}\n"
...
async def is_correct_yes(callback, state: FSMContext):
    ...
    if result == 'Ви вже були успішно зареєстровані.\nБудь ласка, пропишіть /esc і починайте користуватись ботом':
        ...
        await callback.message.edit_text("Чудово! В вашому розпорядженні цілий арсенал команд! Почніть з /m\n"
...
def register_reg_handlers(dp):
    ...
    dp.register_message_handler(get_phone_number, state=FSMuser.phone_number)
